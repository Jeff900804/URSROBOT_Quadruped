...
public:
    ...
    bool requestState(const std::string& state_name)
	{
	    ...
	    requestedStateMode.store(FSMStringMap.right.at(state_name));
	    spdlog::info("FSM: requested state {}", state_name);
	    return true;
	}
private:
	...
	bool changeState_(int nextStateMode)
	{
	    ...
	    spdlog::warn("FSM: requested state id {} not found", nextStateMode);
	    return false;
	}
	...
	if (requested != 0)
	{
	    changeState_(requested);
	    return;
	}

	// 再處理原本 joystick / safety checks
	if (nextStateMode != 0 && !currentState->isState(nextStateMode))
	{
	    changeState_(nextStateMode);
	}
    }
